...
function jobFinished(job) {
    // Original, incorrect implementation:  return job.result !== null;
    var result1 = job.finished_at !== null;
    var result2 = job.state === "finished";
    if (result1 !== result2) {
        log('jobsFinished: inconsistent for job ' + job.number + ' ' + job.id + ': ' + job.finished_at + ', ' + job.state);
    }
    return result1;
}
...
function isLeaderJob(job, jobs) {
    // Could compare IDs instead of the job objects themselves?
