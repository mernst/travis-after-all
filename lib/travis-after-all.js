...
function getJob(jobs, id) {
    ...
    jobs.some(function (job) {
        log('getJob: testing ' + job.id + ' against ' + id + ' results: ' + ( job.id === id ) + ' ' + ( job.id == id ));
        if ( job.id === id ) {
            log('getJob: found result ' + job.number);
            result = job;
            return true;
        }
        return false;
...
function handleJob(jobs) {
    ...
    log('I am ' + current_job + ' with ID ' + travis.CURRENT_JOB_ID);
    log('leader job is ' + jobs[jobs.length-1]);
