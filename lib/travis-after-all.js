...
//  * 1 for the leader job, if the build failed
...
function jobFinished(job) {
    // Don't test job.state, job.status, or job.result, which are undefined
    // if the job was canceled.
    return (job.finished_at !== null);
}

function jobFailed(job) {
    return jobFinished(job)
        && (job.result !== 0)
...
function handleJob(jobs) {
    ...
    if ( allJobsAreFinished(jobs) ) {
        ...
        if ( thereIsASuccessfulJob(jobs) ) {
            ...
            // defined fail tasks.
            log("There is no failing or succeeding job");
            endJob(1);
        }
    }
